public class LinkedList {
    ...
    public boolean isInIncreasingOrder() {
        //  An indirectly recursive method that calls isInIncreasingOrder function
        return isInIncreasingOrder(head);
    }

    public boolean isInIncreasingOrder(Item item) {
        // Check if list has only one item
        if (item == null || head == tail) {
            return true;
        } // Base condition
        if (item.next == tail && item.data <= tail.data) {
            return true;
        } // Recursive call
        if (item.data <= item.next.data) {
            return isInIncreasingOrder(item.next);
        } else {
            return false;
        }
    }
